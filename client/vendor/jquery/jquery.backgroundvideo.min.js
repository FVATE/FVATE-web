  ...
  $.backgroundVideo = function(el, options) {
    ...
    var buildVideo = function () {
      ...
      for(var i=0; i < plugin.settings.types.length; i++) {
        ...
        file = plugin.settings.path + plugin.settings.filename + '.' + plugin.settings.types[i];

        type = '';
        if (plugin.settings.types[i] === 'mp4') {
          // leave type empty..
        } else {
          type = ' type="video/' + plugin.settings.types[i] + '" ';
        }
        html += '<source src="' + file + '" ' + type + ' />';
      }
    ...
    var setProportion = function () {
      ...
      var height = proportion*plugin.settings.height;
      var width = proportion*plugin.settings.width;

      plugin.$videoEl.width(width);
      plugin.$videoEl.height(height);
